...
from .gaussian_entropy_model import Distribution_for_jnd
...
class Weighted_Gaussian(nn.Module):
    def __init__(self, M):
        ...
        self.gaussin_entropy_func = Distribution_for_jnd()

    def forward(self, x, hyper, jnd):
        x = torch.unsqueeze(x, dim=1)
        hyper = torch.unsqueeze(self.conv3(hyper), dim=1)
        x1 = self.conv1(x)
        output = self.conv2(torch.cat((x1, hyper), dim=1))
        p3 = self.gaussin_entropy_func(torch.squeeze(x, dim=1), output, jnd)
        return p3, output
