...
class Distribution_for_jnd(nn.Module):
    def __init__(self):
        super(Distribution_for_jnd, self).__init__()

    def forward(self, x, p_dec, jnd):
        # you can use 3 gaussian
        ...
        likelihood0 = m0.cdf(x + 0.5*jnd_for_quantize) - m0.cdf(x - 0.5*jnd_for_quantize)
        likelihood1 = m1.cdf(x + 0.5*jnd_for_quantize) - m1.cdf(x - 0.5*jnd_for_quantize)
        likelihood2 = m2.cdf(x + 0.5*jnd_for_quantize) - m2.cdf(x - 0.5*jnd_for_quantize)
